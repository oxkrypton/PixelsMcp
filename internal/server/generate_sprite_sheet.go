...
func newGenerateSpriteSheetTool() mcp.Tool {
	return mcp.NewTool(
		...
		mcp.WithDescription("Generate a sprite sheet image from a prompt, action, frame count, layout, optional reference image, optional frame geometry, optional solid background color, optional tuning args, seed, negative prompt, and output_path. Reference images may be http(s) URLs via reference_image or absolute local file paths via reference_path. Save the result on the server. To save into a caller workspace, pass an absolute output_path. The response saved_path is the actual absolute file path written by the server."),
